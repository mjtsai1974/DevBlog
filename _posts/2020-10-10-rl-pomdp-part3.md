...
>
>* $V_{P_{t}(a_{i})}$  
>Let $V_{P_{t}(a_{i})}$ to be the value function induced by policy tree $P_{t}$, for $i=\\{1,2,...,n\\}$, with $a_{i}$ the $i$-th action in the root node, and this value function is of the form:  
